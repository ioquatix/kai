...
namespace Kai {
	namespace Parser {
		...
		void Expressions::mark(Memory::Traversal * traversal) const {
			for (const Expression * expression : _expressions) {
		...
		ParseResult Expressions::parse(Frame * frame, const ParseState & state) const {		
			...
			StringIteratorT current = state.current;
			
			if (_ignore_whitespace) {
				// Eat whitespace:
				current = Parser::parse_whitespace(current, state.end).end();
			}
		...
		void Expressions::add(const Expression * expression) {
...
// MARK: -
		...
		void Expressions::import (Frame * frame) {
			...
			Ref<Expressions> basic = basic_expressions(frame);
			frame->update(frame->sym("expressions"), basic);
		}
